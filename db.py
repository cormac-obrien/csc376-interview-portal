...
def add_user(conn, name, pass_hash, perms):
    ...
    curs = conn.cursor()
    curs.execute("""
    INSERT INTO Users (user_name, user_password, user_perms)
      VALUES (?, ?)""", (name, pass_hash, perms))
    curs.close()
    conn.commit()
...
def create_interview(conn, name, description):
    ...
    curs = conn.cursor()
    curs.execute("""
    INSERT INTO Interviews (interview_name, interview_description)
...
def add_question(conn, interview, text):
    ...
    curs = conn.cursor()
    curs.execute("""
    INSERT INTO Questions (question_interview, question_text)
      VALUES (?, ?)""", (interview, text))
    curs.close()
    conn.commit()
...
def add_answer(conn, user_id, question_id, text):
    ...
    curs = conn.cursor()
    curs.execute("""
    INSERT INTO Answers (answer_user, answer_question,)
