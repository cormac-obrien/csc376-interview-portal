...
def getUser(username, password):
    ...
    row = conn.execute(select + table + "WHERE UserName = ? AND Password = ?",
                       (username, password)).fetchone()

    if (row is not None):
        res = user.User(row['UserID'], row['UserName'], row['UserRoleID'],
                        row['InterviewID'])
        conn.close()
        return res
...
def getUserRole(userRoleID):
    ...
    row = conn.execute(select + table + "WHERE UserRoleID = ?",
                       (str(userRoleID))).fetchone()
...
def getInterview(InterviewID):
    ...
    JOINS = ("INNER JOIN InterviewRelation ir ON ii.InterviewID = ir.InterviewID "
...
def reviewInterview(interviewID):
    ...
    rows = conn.execute(SELECT + FROM + "WHERE InterviewID = ?",
                        (interviewID, )).fetchall()
...
def submitAnswers(ActiveInterview):
    ...
    for Question in ActiveInterview.getQuestions():
        conn.execute(
            "UPDATE InterviewRelation SET UserAnswerID = (?) WHERE InterviewID = ? and QuestionID = ?",
            (Question.getUserAnswerID(), ActiveInterview.getInterviewID(),
...
def makeNewInterview(activeInterview, UserID):
    ...
    IntID = conn.execute(
        "insert into InterviewInfo(InterviewID,InterviewName) values (?,?)",
        (None, activeInterview.getInterviewName())).lastrowid
    for Q in qlist:
        QID = conn.execute(
            "insert into QuestionInfo(QuestionID, QuestionText) values (?,?)",
            (None, Q.getQuestionText())).lastrowid
        conn.execute(
            "insert into InterviewRelation(InterviewID, QuestionID, UserAnswerID) values (?,?,?)",
            (IntID, QID, -1))
        alist = Q.getAnswers()
        for A in alist:
            AID = conn.execute(
                "insert into AnswerInfo(AnswerID, AnswerText) values(?,?)",
                (None, A.getAnswerText())).lastrowid
            conn.execute(
                "insert into QuestionRelation(QuestionID, AnswerID) values (?,?)",
                (QID, AID))
...
def getUsers():
    ...
    for row in rows:
        users.append(
            user.User(row['UserID'], row['UserName'], 4, row['InterviewID']))
    return users
...
def getUserInterviewID(userName):
    ...
    row = conn.execute(select + table + "WHERE UserName = ?",
                       (userName, )).fetchone()
...
def checkIntAssigned(InterviewID):
    ...
    row = conn.execute(select + table + "WHERE InterviewID = ?",
                       (str(InterviewID))).fetchone()
...
def assignUser(InterviewID, UserName):
    ...
    conn.row_factory = sqlite3.Row
    conn.execute(
        "UPDATE UserInformation SET InterviewID = ? WHERE UserName = ?",
